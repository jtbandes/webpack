...
/** @typedef {{ name: string | VariableInfo, rootInfo: string | VariableInfo, getMembers: () => { name: string, optional: boolean }[] }} GetInfoResult */
 ...
 * @param {{ name: string, optional: boolean }[]} membersReversed member info
...
const objectAndMembersToName = (object, membersReversed) => {
	...
	for (let i = membersReversed.length - 1; i >= 0; i--) {
		name +=
			(membersReversed[i].optional ? "?." : ".") + membersReversed[i].name;
	}
	return name;
};
...
class JavascriptParser extends Parser {
	...
	constructor(sourceType = "auto") {
		...
		this.hooks = Object.freeze({
			...
			/** @type {HookMap<SyncBailHook<[import("estree").AssignmentExpression, { name: string, optional: boolean }[]], boolean | void>>} */
			...
			/** Something like "a.b()" */
			/** @type {HookMap<SyncBailHook<[CallExpressionNode, { name: string, optional: boolean }[]], boolean | void>>} */
			callMemberChain: new HookMap(
				() => new SyncBailHook(["expression", "members"])
			),
			/** Something like "a.b().c.d" */
			/** @type {HookMap<SyncBailHook<[ExpressionNode, { name: string, optional: boolean }[], CallExpressionNode, { name: string, optional: boolean }[]], boolean | void>>} */
			...
			/** @type {HookMap<SyncBailHook<[ExpressionNode, { name: string, optional: boolean }[]], boolean | void>>} */
	 ...
	 * @param {function(string, string | ScopeInfo | VariableInfo, function(): { name: string, optional: boolean }[]): any} fallback callback when variable in not handled by hooks
	 ...
	 * @returns {{ members: { name: string, optional: boolean }[], object: ExpressionNode | SuperNode }} member names (reverse order) and remaining object
	 */
	extractMemberExpressionChain(expression) {
		...
		while (expr.type === "MemberExpression") {
			if (expr.computed) {
				...
				members.push({
					name: `${expr.property.value}`,
					optional: expr.optional
				});
			} else {
				if (expr.property.type !== "Identifier") break;
				members.push({ name: expr.property.name, optional: expr.optional });
			}
			expr = expr.object;
		}
		return {
	...
	/** @typedef {{ type: "call", call: CallExpressionNode, calleeName: string, rootInfo: string | VariableInfo, getCalleeMembers: () => { name: string, optional: boolean }[], name: string, getMembers: () => { name: string, optional: boolean }[]}} CallExpressionInfo */
	/** @typedef {{ type: "expression", rootInfo: string | VariableInfo, name: string, getMembers: () => { name: string, optional: boolean }[]}} ExpressionExpressionInfo */
	 ...
	 * @returns {{ name: string, rootInfo: ExportedVariableInfo, getMembers: () => { name: string, optional: boolean }[]}} name info
	 */
